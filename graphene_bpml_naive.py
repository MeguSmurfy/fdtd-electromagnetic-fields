...
def iterator(t, nx, ny, ex, ey, hz, curr, dt, dx, dy, ep, mu, tau, D, lx, ly, sigma_star):
    ...
    Obeys a vertical graphene sheet in the middle.
    ...
    # Implement Hz jump discontinuity and Ex continuity condition
    for i in range(nx):
        hz[i][ny // 2] = hz[i][ny // 2 - 1] + curr[i]
        ex[i][ny // 2] = ex[i][ny // 2 - 1]

    # Update the current
    for i in range(nx):
        curr[i] = (curr[i] + D * dt * ex[i][ny//2]) * tau / (tau + dt)
