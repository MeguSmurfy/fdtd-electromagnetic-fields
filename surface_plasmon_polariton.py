...
def iterator(t, nx, ny, ex, ey, hz, curr, dt, dx, dy, ep, mu, tau, D, lx, ly, sigma_star):
    """
    Update one step in time for the fields Ex, Ey, and Hz according to Maxwell's Equations.
    Choices of constants includes ep, mu, delta step sizes, layer sizes, relaxationtime, and Drude constant.
    Obeys a vertical graphene sheet in the middle with surface plasmon polariton rules.
    ...
    curr = (curr + D * dt * ex[:,ny//2]) * tau / (tau + dt)
    hz[:,ny // 2] = hz[:,ny // 2 - 1] + curr
    ex[:,ny // 2] = ex[:,ny // 2 - 1]

    # Add source term shifted left
    # hz[nx // 2][ny // 2-2] += 10e3 * np.sin(2 * 0.03 * np.pi * t)
    hz[nx // 2][ny // 2-2] += 5e4 * np.sin(2 * 0.01 * np.pi * t)
...
def tester():
    ...
    start = time.time()
    for t in range(nt):
        ex, ey, hz, curr = iterator(t, nx, ny, ex, ey, hz, curr, dt, dx, dy, ep, mu, tau, D, lx, ly, sigma_star)
        print(t)
        if t >= 0:
            plt.imshow(hz, interpolation="lanczos", cmap="viridis", vmin=-2e3, vmax=2e3)
            plt.show()
    end = time.time()
    print(end - start)
